...
HoUrpAovOutputData HoUrpEncodeMaterialAov(HoUrpMaterialSemanticData semantic, half maskWeight)
{
    ...
    // Mask only gates participation; SSS contribution still scales by coverage below.
    ...
    data.sssSource = half4(max(semantic.sssSourceColor, 0.0h) * coverage, saturate(semantic.sssWeight) * coverage);
    return data;
}
