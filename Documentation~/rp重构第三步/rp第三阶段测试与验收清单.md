...
| Debug WorldNormal 天空区域 | `Aov.NormalDepth` 未绘制/天空区域显示 invalid normal 黑，不显示默认法线颜色。 |
| Debug LinearDepth 天空区域 | `Aov.NormalDepth` 未绘制/天空区域显示 far depth 白。 |
...
- `Aov.NormalDepth` clear 从中性法线改为 `(0, 0, 0, 1)`；天空球没有几何法线，但深度按 far depth / infinite background 处理。
- `Aov.NormalDepth.a` 后续收敛为 linear 0..1 depth；未覆盖区域的深度为 1，法线有效性由 RGB 是否为 zero 判断。
...
| AOV 空值/天空区域契约已明确，NormalDepth 天空不伪造默认法线，深度显示无限远 | 通过 |
