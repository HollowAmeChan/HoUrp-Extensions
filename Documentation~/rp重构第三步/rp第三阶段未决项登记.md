...
| AOV clear 机制 | 第三阶段收敛到资源声明层 `TextureDesc.clearBuffer/clearColor`；不保留额外 clear draw/pass | 后续资源增多时复核 RenderGraph clear-on-first-use 与原生 render pass load-action 行为 |
| 天空/未覆盖区域 AOV 语义 | 明确为 no geometry；NormalDepth 使用 invalid normal + far depth `(0,0,0,1)`，不写默认法线 fallback | 后续 Debug UI 可增加 checker/overlay 显示空值区域 |
